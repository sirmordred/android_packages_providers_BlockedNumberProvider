...
public class BlockedNumberProvider extends ContentProvider {
    ...
    private Uri insertBlockedNumber(ContentValues cv) {
        ...
        if (phoneNumber.endsWith("*")) {
            cv.put(BlockedNumberContract.BlockedNumbers.COLUMN_E164_NUMBER, phoneNumber);
        } else {
        ...
        Log.d("MORDRED", String.format("inserted blocked number: %s", cv));
    ...
    private boolean isBlocked(String phoneNumber) {
        ...
        final Cursor c_first = mDbHelper.getReadableDatabase().rawQuery(
                "SELECT " +
                BlockedNumberContract.BlockedNumbers.COLUMN_ORIGINAL_NUMBER +
                " FROM " + BlockedNumberDatabaseHelper.Tables.BLOCKED_NUMBERS +
                " WHERE " + BlockedNumberContract.BlockedNumbers.COLUMN_ORIGINAL_NUMBER +
                " LIKE \"%*\"", null);
        try {
            while (c_first.moveToNext()) {
                final String numrange = c_first.getString(0);
                Log.i("MORDRED", "My block impl, blocked range:" + numrange);
                final String numprefix = numrange.substring(0, numrange.length() - 1);
                if (phoneNumber.startsWith(numprefix)) {
                    Log.i("MORDRED", "My block impl, blocked number:" + phoneNumber);
                    return true;
                }
            }
        } finally {
